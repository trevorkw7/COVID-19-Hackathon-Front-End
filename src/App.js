...
function App() {
  ...
  async function fetchData(latitude, longitude) {
    const url =
      "https://corona-19-hackathon.herokuapp.com/location/stats?lat=" +
      latitude +
      "&lng=" +
      longitude;
  ...
  function renderInstructiions() {
    var temp = [];
    temp[0] =
      apiData["Safe Score"] > 75 && !apiData["Stay Home"] ? "Avoid*" : "No*";
    temp[1] = apiData["Stay Home"]
      ? "(A shelter in place order has been issued in your location.)"
      : "(There are no shelter in place orders in your location. )";
  ...
  return (
    <div className="App">
      ...
      <Center>
        ...
        />
        <Typography color="inherit" style={{ paddingRight: "40px" }}>
          OR
        </Typography>
        <AutoCompleteSearch
          className={classes.searchButton}
          onChildClick={handleAddressClick}
        />
        {/* </div> */}
      </Center>
      ...
      <Center>
        {sentApi != null ? (
          <Typography>Your County: {apiData["County"]} </Typography>
        ) : null}
      </Center>
